...
def train_one_epoch(data_loader: Iterable,
    ...
    for batch in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with torch.cuda.amp.autocast(fp16):
            # check if NVIB model
            if  model.module.backbone.__class__.__name__ == "NvibVisionTransformer":
        ...
        if  model.module.backbone.__class__.__name__ == "NvibVisionTransformer":
            klg_ele_sum=0
            kld_ele_sum=0
            for klg_ele,kld_ele in zip(klg,kld):
                klg_ele_sum += klg_ele.mean()
                kld_ele_sum += kld_ele.mean()
            klg=klg_ele_sum/len(klg)
            kld = kld_ele_sum / len(kld)
            loss += klg * lambda_klg + kld * lambda_kld
            if utils.is_main_process():
                wandb.log({"klg_loss": klg * lambda_klg})
                wandb.log({"kld_loss": kld * lambda_kld})
        ...
        if fp16:
            ...
            loss.backward()
            optimizer.step()
        print(model.module.backbone.pos_embed.grad)
        ...
        if utils.is_main_process():
            wandb.log({"loss": loss_value.item(), "lr": lr})
